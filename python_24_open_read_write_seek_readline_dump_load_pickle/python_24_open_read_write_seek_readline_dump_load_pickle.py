import pickle                  # для работы с бинарными данными подключим специальный встроенный модуль pickle
                               # ерез методы этого модуля будем работать с бинарными данными
...
try:
   file3 = open("out.bin", "wb")                   # Откроем файл с расширением .bin - с режимом доступа wb, на запись в бинарном режиме

   try:
       pickle.dump(books, file3)                   # Для записи в файл используется метод dump (указываем что записываем - books, уда записываем - file)
                                                   # передаем ссылку на открытый файл "wb"
                                                   # появляется файл out.bin
   finally:
       file3.close()
